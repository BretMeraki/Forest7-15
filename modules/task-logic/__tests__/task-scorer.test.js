...
describe('TaskScorer', () => {
  ...
  // Critical tests to verify no NaN calculations
  describe('NaN Prevention Tests', () => {
    ...
    test('should never return NaN for task scoring', () => {
      const score = TaskScorer.calculateTaskScore(
        basicTask,
        3, // energyLevel
        30, // timeInMinutes
    ...
    test('should return valid scores for all SCORING constants', () => {
      // Test ADAPTIVE_TASK_BOOST
      const lifeChangeContext = 'lost savings, zero budget';
      const adaptiveScore = TaskScorer.calculateTaskScore(
        adaptiveTask,
        3,
        30,
        lifeChangeContext,
      ...
      expect(adaptiveScore).not.toBeNaN();
      expect(adaptiveScore).toBeGreaterThan(basicTask.priority + SCORING.ADAPTIVE_TASK_BOOST);

      // Test MOMENTUM_TASK_BASE_BOOST
      const momentumScore = TaskScorer.calculateTaskScore(
        momentumTask,
        4,
        120,
        '',
      ...
      expect(momentumScore).toBeGreaterThan(momentumTask.priority + SCORING.MOMENTUM_TASK_BASE_BOOST);

      // Test BREAKTHROUGH_AMPLIFICATION_BONUS
      const breakthroughScore = TaskScorer.calculateTaskScore(
        breakthroughTask,
        5,
        45,
        '',
      ...
      expect(breakthroughScore).not.toBeNaN();
      expect(breakthroughScore).toBeGreaterThan(breakthroughTask.priority + SCORING.BREAKTHROUGH_AMPLIFICATION_BONUS);
    });

    test('should handle edge cases without NaN', () => {
      // Test with undefined/null inputs
      const edgeScore1 = TaskScorer.calculateTaskScore(
        { ...basicTask, priority: undefined, difficulty: null },
        3,
        30,
        '',
        projectContext
      ...
      expect(edgeScore1).not.toBeNaN();
      expect(typeof edgeScore1).toBe('number');

      // Test with extreme values
      const edgeScore2 = TaskScorer.calculateTaskScore(
        { ...basicTask, priority: 0, difficulty: 10 },
        1,
        1,
        '',
        projectContext
      ...
      expect(edgeScore2).not.toBeNaN();
      expect(typeof edgeScore2).toBe('number');
    });

    test('should verify all SCORING constants are numbers', () => {
      expect(typeof SCORING.ADAPTIVE_TASK_BOOST).toBe('number');
      expect(typeof SCORING.TIME_FIT_BONUS).toBe('number');
      expect(typeof SCORING.TIME_ADAPT_BONUS).toBe('number');
      expect(typeof SCORING.TIME_TOO_LONG_PENALTY).toBe('number');
      expect(typeof SCORING.DOMAIN_RELEVANCE_BONUS).toBe('number');
      expect(typeof SCORING.CONTEXT_RELEVANCE_BONUS).toBe('number');
      expect(typeof SCORING.MOMENTUM_TASK_BASE_BOOST).toBe('number');
      expect(typeof SCORING.BREAKTHROUGH_AMPLIFICATION_BONUS).toBe('number');
      expect(typeof SCORING.GENERATED_TASK_BOOST).toBe('number');

      expect(SCORING.ADAPTIVE_TASK_BOOST).not.toBeNaN();
      expect(SCORING.TIME_FIT_BONUS).not.toBeNaN();
      expect(SCORING.TIME_ADAPT_BONUS).not.toBeNaN();
      expect(SCORING.TIME_TOO_LONG_PENALTY).not.toBeNaN();
      expect(SCORING.DOMAIN_RELEVANCE_BONUS).not.toBeNaN();
      expect(SCORING.CONTEXT_RELEVANCE_BONUS).not.toBeNaN();
      expect(SCORING.MOMENTUM_TASK_BASE_BOOST).not.toBeNaN();
      expect(SCORING.BREAKTHROUGH_AMPLIFICATION_BONUS).not.toBeNaN();
      expect(SCORING.GENERATED_TASK_BOOST).not.toBeNaN();
    });
  });
});